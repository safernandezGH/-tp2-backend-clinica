...
export async function crearEspecialidad(req, res) {
  try {
    ...
    const especialidadId = resultado.insertId;
    ...
    return respuestaOk(res, 201, { id: especialidadId, descripcion });
  } catch (error) {
    console.error(error);
...
export async function actualizarEspecialidad(req, res) {
  try {
    ...
    await registrarAuditoria(
      ...
      "especialidad",
      Number(id),
      `Modificación de especialidad: ${descripcion}`
    );
...
export async function eliminarEspecialidad(req, res) {
  try {
    const { id } = req.params;
    const [especialidades] = await pool.query("SELECT id, descripcion FROM especialidad WHERE id = ?", [id]);
    if (especialidades.length === 0) {
      return respuestaError(res, 404, "Especialidad no encontrada");
    }
